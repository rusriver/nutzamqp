...
func AMQPBatchDeclare(
	...
	re_ddash_dot, err := regexp.Compile(`\-\-`); if err != nil { panic(fmt.Errorf("!!! nutzamqp.go:19 / 0.1 at \"re_ddash_dot, err := regexp.Compile(`\-\-`)\": %v", err)) }

	log.Fields["lib"] = "nutzamqp.AMQPBatchDeclare"

	for k, _ := range cfg.UMap("exchanges") {
	
		x, err := cfg.Get("exchanges." + k); if err != nil { panic(fmt.Errorf("!!! nutzamqp.go:26 / 1.1 at \"x, err := cfg.Get(\"exchanges.\" + k)\": %v", err)) }
		...
		err = channel.ExchangeDeclare(
			...
			false,		// noWait
			nil,		// arguments
		); if err != nil { panic(fmt.Errorf("!!! nutzamqp.go:38 / 1.2: %v", err)) }
		
		log.Info("exchange declared OK: %v", k)
	}
	for k, _ := range cfg.UMap("queues") {

		q, err := cfg.Get("queues." + k); if err != nil { panic(fmt.Errorf("!!! nutzamqp.go:46 / 2.1 at \"q, err := cfg.Get(\"queues.\" + k)\": %v", err)) }
		...
		_, err = channel.QueueDeclare(
			...
			false,		// noWait
			nil,		// arguments
		); if err != nil { panic(fmt.Errorf("!!! nutzamqp.go:57 / 2.2: %v", err)) }
	...
	for _, v := range cfg.UList("bindings") {
		...
		err = channel.QueueBind(
			...
			false,	// noWait
			nil,	// arguments
		); if err != nil { panic(fmt.Errorf("!!! nutzamqp.go:76 / 3.2: %v", err)) }
